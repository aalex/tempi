...
namespace tempi_osc_looper {

// String constants:
static const char *PROGRAM_NAME = "tempi-osc-looper";
...
class App
{
    ...
    private:
        ...
        bool recording_;
        bool playing_;
        unsigned int osc_input_port_;
        unsigned int osc_control_port_;
        unsigned int osc_output_port_;
        std::string osc_output_host_;
        bool graph_ok_;
...
bool App::setupGraph()
{
    ...
    graph->setNodeAttribute("osc.recv0", "port", tempi::Message("i", osc_input_port_));
    graph->setNodeAttribute("osc.send0", "host_port", tempi::Message("si", osc_output_host_.c_str(), osc_output_port_));
...
int App::parse_options(int argc, char **argv)
{
    ...
    desc.add_options()
        ...
        ("input,i", po::value<unsigned int>()->default_value(0),
            "Sets the OSC input port to listen to")
        //("control,c", po::value<unsigned int>()->default_value(0),
    ...
    osc_input_port_ = options["input"].as<unsigned int>();
    osc_output_port_ = options["output"].as<unsigned int>();
    osc_output_host_ = options["host"].as<std::string>();
    if (verbose_)
    {
        std::cout << "OSC input port: " << osc_input_port_ << std::endl;
        std::cout << "OSC output: " << osc_output_host_ << ":" <<
            osc_output_port_ << std::endl;
    }
...
int main(int argc, char *argv[])
{
    using namespace tempi_osc_looper;
    int ret;
    App app;
    try
    {
